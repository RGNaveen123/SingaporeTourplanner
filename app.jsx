...
function TripSetupSheet({ open, initial, homeBaseInitial, onSave, onClose, dismissable }) {
  ...
  const locate = async () => {
    ...
    if (shortLink) {
      ...
      setResolving(false);
      if (resolved) { setResolvedCoords(resolved); setResolveMsg("Resolved your shortened link — check the pin looks right."); return; }
    }
    if (!hbName.trim()) {
...
function AddSpotForm({ onAdd }) {
  ...
  const locate = async () => {
    ...
    if (isShortMapsLink(url)) {
      ...
      if (resolved) { setFoundCoords(resolved); setZone(nearestZoneForCoords(resolved) || zone); setGeocodeMsg("Resolved your shortened link — check the pin looks right."); setGeocoding(false); return; }
    }
    setGeocoding(true);
    ...
    else setGeocodeMsg("Couldn't find an exact spot for that — it'll still save, just without a precise map pin. Try a more specific name, or paste a full (non-shortened) Maps link instead.");
  };
  const submit = () => {
    if (!name.trim()) return;
