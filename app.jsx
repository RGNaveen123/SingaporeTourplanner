...
function isShortMapsLink(url) { return /goo\.gl\/maps|maps\.app\.goo\.gl/i.test(url || ""); }
// Shortened Google Maps links (maps.app.goo.gl, goo.gl/maps) don't contain
// coordinates — only Google's own redirect reveals them, and a browser
// can't follow that cross-origin on its own (no CORS headers on Google's
// side). This uses a free, third-party public relay (allorigins.win) that
// fetches the link server-side and hands back the resolved page, so we can
// read the real coordinates out of either the final URL or the page
// content. It's a best-effort step, not something this app controls or can
// guarantee — if the relay is ever slow/down, this just returns null and
// the caller falls through to free name-based geocoding instead.
async function resolveShortMapsLink(shortUrl) {
  try {
    const proxied = `https://api.allorigins.win/get?url=${encodeURIComponent(shortUrl)}`;
    const res = await fetch(proxied, { signal: AbortSignal.timeout ? AbortSignal.timeout(8000) : undefined });
    if (!res.ok) return null;
    const json = await res.json();
    const fromFinalUrl = extractCoordsFromUrl(json?.status?.url || "");
    if (fromFinalUrl) return fromFinalUrl;
    const fromHtml = extractCoordsFromUrl(json?.contents || "");
    if (fromHtml) return fromHtml;
  } catch (e) {}
  return null;
}
// Last-resort location fallback: give Claude the link/name/description and
// let it use REAL web search (server-side — no CORS problem at all, unlike
// the tricks above) to figure out the actual place, then hand back a plain
// address/area that the free Nominatim geocoder can resolve to coordinates.
// This is explicit and opt-in (a labeled button, never automatic) because
// it costs more than a plain text call — a search-enabled call, not just
// tokens. Always try the free paths first; this is the "still stuck" button.
async function askAiToLocate(query) {
  const system = `You help pin down a real physical location in Singapore from a messy description — it might be a Google Maps link (possibly shortened), a place name, a person's guest house, or a vague area like "near Little India". Use web search if it helps. Respond with ONLY raw JSON, no prose, no markdown fences: {"guess": string or null, "confidence": "high"|"low"}. "guess" should be your best specific search-engine-friendly text for a geocoder — a street address, building name, or at minimum a real neighbourhood/landmark name in Singapore (e.g. "Little India, Singapore" or "Race Course Road, Singapore"). Use null only if you truly have nothing to go on.`;
  const tools = [{ type: "web_search_20250305", name: "web_search" }];
  const raw = await askClaude(query, system, 500, tools);
  try {
    const parsed = JSON.parse(stripFences(raw));
    return parsed && parsed.guess ? parsed.guess : null;
  } catch (e) { return null; }
}
...
async function askClaude(promptOrMessages, systemPrompt, maxTokens = 1000, tools = null) {
  const key = getApiKey();
  ...
  bumpCallCount();
  const body = { model: "claude-haiku-4-5-20251001", max_tokens: maxTokens, system: systemPrompt, messages };
...
function GlobalStyle() {
  return (
    <style>{`
      ...
      .sgtp-display { font-family: 'Big Shoulders Display', 'Inter', sans-serif; font-weight: 700; letter-spacing: -0.02em; line-height: 1.05; }
      .sgtp-mono { font-family: 'IBM Plex Mono', monospace; }

      /* Motion tokens — exact values, not approximated. --ease-out and
         --ease-in-out are the strong UI curves; --ease-drawer is the
         iOS-style sheet curve (Ionic). Durations follow the same tables:
         button press 160ms, drawers 200–500ms, stagger 300ms/50ms steps. */
      .sgtp-root {
      ...
      @keyframes sgtp-stagger-in { from { opacity: 0; transform: translateY(8px);} to { opacity: 1; transform: translateY(0);} }
      ...
      @keyframes sgtp-fade-only { from { opacity: 0; } to { opacity: 1; } }

      /* Tab-switch / section entrance: seen many times a session, so kept
         fast and subtle (200ms) rather than a showy slide. */
      .sgtp-enter { animation: sgtp-fade-in 200ms var(--ease-out) both; }
      .sgtp-sheet-enter { animation: sgtp-sheet-up 400ms var(--ease-drawer) both; }
      .sgtp-sheet-exit { animation: sgtp-sheet-down 300ms var(--ease-drawer) both; }
      .sgtp-scrim-enter { animation: sgtp-scrim-in 250ms var(--ease-out) both; }
      .sgtp-pop { animation: sgtp-pop 400ms var(--ease-out) both; }
      .sgtp-glow { animation: sgtp-glow 1.1s var(--ease-out) 1; }
      .sgtp-removing { animation: sgtp-collapse 250ms var(--ease-out) forwards; overflow: hidden; }
      ...
      /* Button press: instant feedback on pointer-down (:active fires on
         press, not release), 160ms per the recipe — scale() carries the
         label/icon with it, which reads as a physical press. */
      .sgtp-btn { transition: transform 160ms var(--ease-out); }
      .sgtp-btn:active { transform: scale(0.97); }
      /* Staggered card entrance: apply .sgtp-stagger to a list wrapper and
         ...
         300ms, 50ms steps, matching the stagger recipe exactly. */
      .sgtp-stagger > * { opacity: 0; animation: sgtp-stagger-in 300ms var(--ease-out) forwards; }
      .sgtp-stagger > *:nth-child(1) { animation-delay: 0ms; }
      .sgtp-stagger > *:nth-child(2) { animation-delay: 50ms; }
      .sgtp-stagger > *:nth-child(3) { animation-delay: 100ms; }
      .sgtp-stagger > *:nth-child(4) { animation-delay: 150ms; }
      .sgtp-stagger > *:nth-child(5) { animation-delay: 200ms; }
      .sgtp-stagger > *:nth-child(n+6) { animation-delay: 250ms; }

      /* Reduced motion: gentler, not zero — cross-fade instead of sliding
         or scaling, per prefers-reduced-motion guidance. */
      @media (prefers-reduced-motion: reduce) {
        .sgtp-enter, .sgtp-sheet-enter, .sgtp-sheet-exit, .sgtp-scrim-enter, .sgtp-stagger > * {
          animation: sgtp-fade-only 200ms ease both !important;
          transform: none !important;
        }
        .sgtp-btn, .sgtp-btn:active { transition: opacity 160ms ease; transform: none !important; }
      }
      /* Reduced transparency is a distinct signal from reduced motion —
         raise translucent chrome to near-solid and drop the blur. */
      @media (prefers-reduced-transparency: reduce) {
        .sgtp-glass { background: ${C.ink} !important; backdrop-filter: none !important; }
      }
...
function Sheet({ open, onClose, title, children, dismissable = true }) {
  ...
  const drag = useRef({ active: false, startY: 0, startTime: 0, lastY: 0 });
  const reduceMotion = useReducedMotion();
  useEffect(() => {
    if (open) { setRendered(true); setClosing(false); setDragY(0); }
    else if (rendered) { setClosing(true); const t = setTimeout(() => setRendered(false), 300); return () => clearTimeout(t); }
  }, [open]); // eslint-disable-line
  if (!rendered) return null;

  // Drag-to-dismiss on the handle/header only — deliberately not the whole
  // body, so it never fights a long scrollable sheet's own scroll gesture.
  // 1:1 tracking (the sheet follows the finger exactly, not a delayed
  // animation), rubber-band resistance if dragged past fully-open, and a
  // flick counts even short of the distance threshold — same shape as any
  // native bottom sheet.
  const onPointerDown = (e) => {
    if (!dismissable || reduceMotion) return;
    drag.current = { active: true, startY: e.clientY, startTime: Date.now(), lastY: e.clientY };
    setIsDragging(true);
  ...
  const onPointerMove = (e) => {
    ...
    setDragY(delta < 0 ? delta * 0.3 : delta); // resist dragging "past open"; free below 0
  };
  const endDrag = () => {
    if (!drag.current.active) return;
    const delta = drag.current.lastY - drag.current.startY;
    const timeTaken = Math.max(1, Date.now() - drag.current.startTime);
    const velocity = Math.abs(delta) / timeTaken;
    drag.current.active = false;
    setIsDragging(false);
    setDragY(0);
    if (delta > 120 || (delta > 40 && velocity > 0.11)) onClose && onClose();
  };
  return (
    <div className="sgtp-scrim-enter" style={{ position: "absolute", inset: 0, background: "rgba(5,15,14,0.65)", backdropFilter: "blur(2px)", zIndex: 10000, display: "flex", alignItems: "flex-end" }} onClick={() => dismissable && onClose && onClose()}>
      <div
        className={closing ? "sgtp-sheet-exit" : "sgtp-sheet-enter"}
        onClick={(e) => e.stopPropagation()}
        style={{
          width: "100%", maxHeight: "86%", overflowY: "auto",
          background: "linear-gradient(180deg, #14403A 0%, #0F332F 100%)", borderTop: `1px solid ${C.line}`,
          borderRadius: "20px 20px 0 0", padding: "10px 20px 28px", boxShadow: "0 -20px 50px rgba(0,0,0,0.45)",
          transform: dragY ? `translateY(${dragY}px)` : undefined,
          transition: isDragging ? "none" : "transform 400ms var(--ease-drawer)",
        }}
      >
        <div onPointerDown={onPointerDown} onPointerMove={onPointerMove} onPointerUp={endDrag} onPointerCancel={endDrag} style={{ touchAction: dismissable ? "none" : "auto", cursor: dismissable && !reduceMotion ? "grab" : "default" }}>
...
function TripSetupSheet({ open, initial, homeBaseInitial, onSave, onClose, dismissable }) {
  ...
  const [freeAttempted, setFreeAttempted] = useState(false);
  const [aiLocating, setAiLocating] = useState(false);
  ...
  // Resolve coords, in order: (1) a FULL Maps URL with lat/lng embedded —
  // instant, no network; (2) a shortened maps.app.goo.gl/goo.gl link — via
  // the best-effort relay above; (3) free Nominatim geocoding on the typed
  // name — same path "Add your own spot" already uses.
  const locate = async () => {
    setResolveMsg(""); setResolvedCoords(null); setAiErr("");
    const urlCoords = extractCoordsFromUrl(hbUrl);
    if (urlCoords) { setResolvedCoords(urlCoords); setResolveMsg("Found it from your link."); setFreeAttempted(false); return; }
    const shortLink = isShortMapsLink(hbUrl);
    if (shortLink) {
      setResolving(true);
      const resolved = await resolveShortMapsLink(hbUrl);
      setResolving(false);
      if (resolved) { setResolvedCoords(resolved); setResolveMsg("Resolved your shortened link — check the pin looks right."); setFreeAttempted(false); return; }
    }
    if (!hbName.trim()) {
      setResolveMsg(shortLink
        ? "Couldn't resolve that short link right now (it depends on an outside service that's occasionally slow). Type the place or street name above too and I'll search for it instead, or open the link in a browser and paste the full resulting URL."
      ...
      setFreeAttempted(true);
      return;
    }
    setResolving(true);
    const geo = await geocodeAddress(hbName);
    setResolving(false);
    if (geo) { setResolvedCoords(geo); setResolveMsg("Found it by name — check the pin looks right."); setFreeAttempted(false); }
    else {
      setResolveMsg(shortLink
        ? "Couldn't resolve the short link or find that name for free. Try a more specific nearby landmark — or use AI search below."
        : "Couldn't find that for free — try a more specific name, a full address, a well-known nearby landmark, or use AI search below.");
      setFreeAttempted(true);
    }
  };
  // Last resort: hand the name/link to Claude with real web search enabled.
  // Explicit button, never automatic — this costs more than a plain call.
  const tryAi = async () => {
    setAiErr(""); setAiLocating(true);
    try {
      const query = [hbName, hbUrl].filter(Boolean).join(" — ") || "unknown location";
      const guess = await askAiToLocate(query);
      if (guess) {
        const geo = await geocodeAddress(guess);
        if (geo) { setResolvedCoords(geo); setResolveMsg(`AI found "${guess}" — check the pin looks right.`); setFreeAttempted(false); }
        else setResolveMsg(`AI's best guess was "${guess}" — type that in the name field above and tap Find on map to try locating it directly.`);
      } else {
        setResolveMsg("AI couldn't narrow it down either — try a more specific name or a well-known nearby landmark instead.");
      }
    } catch (e) {
      setAiErr(e.code === "no-api-key" ? "Add your Anthropic API key in Settings (key icon) first." : e.code === "budget-declined" ? "No problem — skipped that call." : "Couldn't reach AI search — check connection and try again.");
    } finally { setAiLocating(false); }
  };
  ...
  return (
    <Sheet open={open} onClose={onClose} title="Trip setup" dismissable={dismissable}>
      ...
      <Field label="Where you're staying (guest house / relative's place)">
        <input className="sgtp-input" style={{ fontFamily: "Inter", marginBottom: 8 }} placeholder="e.g. Aunty Su's place, Toa Payoh — or a nearby landmark's name" value={hbName} onChange={(e) => { setHbName(e.target.value); setResolvedCoords(null); setResolveMsg(""); setFreeAttempted(false); setAiErr(""); }} />
        <input className="sgtp-input" style={{ fontFamily: "Inter" }} placeholder="Paste Google Maps link (full URL works best, not a shortened share link)" value={hbUrl} onChange={(e) => { setHbUrl(e.target.value); setResolvedCoords(null); setResolveMsg(""); setFreeAttempted(false); setAiErr(""); }} />
      </Field>
      ...
      {freeAttempted && !resolvedCoords && (
        <>
          <button onClick={tryAi} disabled={aiLocating} className="sgtp-btn sgtp-mono" style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, background: "linear-gradient(135deg, rgba(227,177,92,0.16), rgba(227,177,92,0.04))", border: `1px solid ${C.gold}`, color: C.gold, borderRadius: 10, padding: "10px", fontSize: 12.5, cursor: aiLocating ? "default" : "pointer", marginBottom: 8 }}>
            {aiLocating ? <Icon name="spinner" size={14} /> : <Icon name="sparkles" size={14} />}
            {aiLocating ? "Searching…" : "Still stuck? Ask AI to find it (uses web search, costs a bit more)"}
          </button>
          {aiErr && <div style={{ fontSize: 12, color: C.danger, marginBottom: 12 }}>{aiErr}</div>}
        </>
      )}
...
function ScheduleSheet({ open, onClose, place, tripLength, arrivalDateStr, dayTags, itinerary, homeBase, onConfirm }) {
  ...
  // would already have ended by your chosen start time; overridable. Works
  // fine with zero options too — the map/reference section just doesn't
  // show until there's at least one real point to reference from.
  const fromOptions = [];
  ...
  return (
    <Sheet open={open} onClose={onClose} title={`Schedule · ${place.name}`}>
      <Field label="Day">
        <select className="sgtp-select" value={day} onChange={(e) => setDay(Number(e.target.value))}>
          {dayOptions.map((d) => {
            ...
            return <option key={d} value={d}>Day {d} — {fmtDateLabel(dateStr)}{tag ? ` (${tag})` : ""}</option>;
          })}
        </select>
      </Field>
      {!homeBase?.coords && (
        <p style={{ fontSize: 12, color: C.muted, marginTop: -6, marginBottom: 14, lineHeight: 1.5 }}>Tip: add your stay location in Trip setup (pencil icon) to get automatic travel-time estimates here — totally optional, you can schedule without it.</p>
      )}
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
      ...
      <PrimaryButton onClick={handleConfirm} icon={<Icon name="plus" size={16} />}>Add to itinerary</PrimaryButton>
    </Sheet>
  );
}
...
function ItineraryTab({ data, setData, recentlyAddedIds, removingIds, setRemovingIds }) {
  ...
  const openDayCount = days.filter((d) => dayTags[d] !== "Universal Studios").length;
  ...
  const handleAutoPlan = async () => {
    ...
    try {
      ...
      const context = {
        homeBaseZone: homeBase?.zone || null,
...
- If homeBaseZone is null, the stay location isn't set yet — still plan normally, just don't assume a specific starting zone.
- Output ONLY a raw JSON array, no markdown fences, no prose. Each item: {"day":number,"name":string,"startTime":"HH:MM","durationMin":number,"zone":"Central|Orchard|Sentosa|Mandai|Changi|Other","notes":string}. Keep notes under 12 words. Keep the array reasonably short.`;
  ...
  return (
    <div className="sgtp-enter" style={{ padding: "16px 16px calc(var(--sgtp-nav-h, 64px) + 36px)" }}>
      ...
      {!homeBase?.coords && (
        <p style={{ fontSize: 12, color: C.muted, marginBottom: 16, lineHeight: 1.5 }}>Tip: add your stay location (pencil icon) for automatic travel-time estimates between stops — totally optional, everything below works without it.</p>
      )}
      ...
      {days.map((d) => {
        ...
        // the first scheduled stop. Purely additive: no home base yet just
        // means this line doesn't show, nothing else is affected.
        const showStayConnector = !!homeBase?.coords && d !== 1 && items.length > 0 && items[0].zone;
        ...
        return (
          <div key={d} className="sgtp-enter" style={{ marginBottom: 28 }}>
            ...
            {items.length === 0 ? (
              <div style={{ color: C.muted, fontSize: 13, padding: "10px 2px", fontStyle: "italic" }}>Nothing scheduled yet — add from Places, or use auto-plan above.</div>
            ) : (
...
function AddSpotForm({ onAdd }) {
  ...
  const [aiLocating, setAiLocating] = useState(false);
  const [aiErr, setAiErr] = useState("");
  // Try the pasted link first (instant, no network); if it's a shortened
  // link, try resolving it server-side; then fall back to free Nominatim
  // geocoding on the typed name — so a plain address works too.
  const locate = async () => {
    if (!name.trim()) return;
    setGeocodeMsg(""); setFoundCoords(null); setAiErr("");
    const urlCoords = extractCoordsFromUrl(url);
    if (urlCoords) { setFoundCoords(urlCoords); setZone(nearestZoneForCoords(urlCoords) || zone); setGeocodeMsg("Found it from your link."); setFreeAttempted(false); return; }
    if (isShortMapsLink(url)) {
      setGeocoding(true);
      const resolved = await resolveShortMapsLink(url);
      if (resolved) { setFoundCoords(resolved); setZone(nearestZoneForCoords(resolved) || zone); setGeocodeMsg("Resolved your shortened link — check the pin looks right."); setGeocoding(false); setFreeAttempted(false); return; }
    }
    setGeocoding(true);
    const geo = await geocodeAddress(name);
    setGeocoding(false);
    if (geo) { setFoundCoords(geo); setZone(nearestZoneForCoords(geo) || zone); setGeocodeMsg("Found it — check the pin looks right."); setFreeAttempted(false); }
    else { setGeocodeMsg("Couldn't find an exact spot for that for free — it'll still save without a precise pin, or try AI search below."); setFreeAttempted(true); }
  };
  const tryAi = async () => {
    setAiErr(""); setAiLocating(true);
    try {
      const query = [name, url].filter(Boolean).join(" — ");
      const guess = await askAiToLocate(query);
      if (guess) {
        const geo = await geocodeAddress(guess);
        if (geo) { setFoundCoords(geo); setZone(nearestZoneForCoords(geo) || zone); setGeocodeMsg(`AI found "${guess}" — check the pin looks right.`); setFreeAttempted(false); }
        else setGeocodeMsg(`AI's best guess was "${guess}" — try typing that as the name above and tap Find on map.`);
      } else {
        setGeocodeMsg("AI couldn't narrow it down either — it'll still save without a precise pin.");
      }
    } catch (e) {
      setAiErr(e.code === "no-api-key" ? "Add your Anthropic API key in Settings (key icon) first." : e.code === "budget-declined" ? "No problem — skipped that call." : "Couldn't reach AI search — check connection and try again.");
    } finally { setAiLocating(false); }
  ...
  return (
    <div className="sgtp-enter" style={{ background: C.panel, border: `1px solid ${C.line}`, borderRadius: 14, padding: 16, marginTop: 6 }}>
      <Field label="Name"><input className="sgtp-input" style={{ fontFamily: "Inter" }} value={name} onChange={(e) => { setName(e.target.value); setFoundCoords(null); setGeocodeMsg(""); setFreeAttempted(false); setAiErr(""); }} placeholder="e.g. Aunty's favourite laksa stall" /></Field>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
      ...
      <Field label="Google Maps link (optional — helps it find the exact spot)"><input className="sgtp-input" style={{ fontFamily: "Inter" }} value={url} onChange={(e) => { setUrl(e.target.value); setFoundCoords(null); setGeocodeMsg(""); setFreeAttempted(false); setAiErr(""); }} placeholder="Paste a share link, or leave blank to search by name" /></Field>
      ...
      {freeAttempted && !foundCoords && (
        <>
          <button onClick={tryAi} disabled={aiLocating} className="sgtp-btn sgtp-mono" style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, background: "linear-gradient(135deg, rgba(227,177,92,0.16), rgba(227,177,92,0.04))", border: `1px solid ${C.gold}`, color: C.gold, borderRadius: 10, padding: "10px", fontSize: 12.5, cursor: aiLocating ? "default" : "pointer", marginBottom: 10 }}>
            {aiLocating ? <Icon name="spinner" size={14} /> : <Icon name="sparkles" size={14} />}
            {aiLocating ? "Searching…" : "Still stuck? Ask AI to find it (uses web search, costs a bit more)"}
          </button>
          {aiErr && <div style={{ fontSize: 12, color: C.danger, marginBottom: 10 }}>{aiErr}</div>}
        </>
      )}
...
TRIP JSON (homeBase.coords may be null if the stay location isn't set yet — that's fine, scheduling still works, you just won't have a precise starting-point zone for travel-time context):
${JSON.stringify(data)}
...
- Don't restate the changes in your visible reply — a confirmation list is shown separately, so keep your prose to context/reasoning only.`;
      ...
      let changes = [];
      if (actions.length > 0) {
        const { newItinerary, results } = applyAiActions(actions, { itinerary: data.itinerary, tripLength, placePool });
        changes = results;
...
function Header({ tripSetup, homeBase, onEdit, onSettings, onExpenses }) {
  const tripLength = tripSetup ? dayNumberFor(tripSetup.departureDate, tripSetup.arrivalDate) : 0;
  return (
    <div className="sgtp-glass" style={{ padding: "calc(env(safe-area-inset-top, 0px) + 14px) 18px 14px", background: "rgba(10,35,32,0.85)", backdropFilter: "blur(14px) saturate(160%)", borderBottom: `1px solid ${C.line}`, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10 }}>
...
function BottomNav({ active, setActive, navRef }) {
  ...
  return (
    <div ref={navRef} className="sgtp-glass" style={{ position: "absolute", left: 0, right: 0, bottom: 0, zIndex: 9999, background: "rgba(10,35,32,0.92)", backdropFilter: "blur(10px)", borderTop: `1px solid ${C.line}`, padding: "8px 4px calc(env(safe-area-inset-bottom, 0px) + 8px)", display: "flex", justifyContent: "space-around" }}>
      {tabs.map((t) => {
