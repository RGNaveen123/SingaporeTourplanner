...
const TAG_ORDER = ["Budget Food", "Sight", "Viewpoint", "Nature", "Culture", "Family", "Shopping", "Food", "Custom"];
...
function SettingsSheet({ open, onClose }) {
  ...
  useEffect(() => { if (open) { setKey(getApiKey()); setCallCount(getCallCount()); } }, [open]);
...
function loadExpenses() { try { return JSON.parse(localStorage.getItem(EXPENSES_KEY) || "[]"); } catch (e) { return []; } }
...
function ExpensesSheet({ open, onClose }) {
  ...
  const [type, setType] = useState("mrt");
  const [amount, setAmount] = useState("");
  const [label, setLabel] = useState("");
  const [splitType, setSplitType] = useState("personal");
  useEffect(() => { if (open) setEntries(loadExpenses()); }, [open]);
  ...
  const add = () => {
    ...
    if (!amt || amt <= 0) return;
    const entry = { id: uid(), type, amount: amt, label: label.trim(), splitType, ts: Date.now() };
    const next = [entry, ...entries];
    setEntries(next); saveExpenses(next);
    setAmount(""); setLabel("");
  };
  const remove = (id) => { const next = entries.filter((e) => e.id !== id); setEntries(next); saveExpenses(next); };
  const fmtMoney = (n) => `S$${n.toFixed(2)}`;
  return (
    <Sheet open={open} onClose={onClose} title="Expenses">
      <p style={{ color: C.muted, fontSize: 13, marginTop: -6, marginBottom: 16, lineHeight: 1.5 }}>
        Quick-log MRT/bus fares and other spend as you go. Purely local bookkeeping — no AI, no cost. Use this alongside whatever app your group already uses to actually settle up.
      </p>
      ...
      <Field label="Type">
        <div style={{ display: "flex", gap: 6 }}>
          {EXPENSE_TYPES.map((t) => (
            <button key={t.id} onClick={() => setType(t.id)} className="sgtp-btn" style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 4, padding: "9px 4px", borderRadius: 10, cursor: "pointer", border: `1px solid ${type === t.id ? C.gold : C.line}`, background: type === t.id ? "rgba(227,177,92,0.14)" : "transparent", color: type === t.id ? C.gold : C.muted }}>
              <Icon name={t.icon} size={16} /><span className="sgtp-mono" style={{ fontSize: 10 }}>{t.label}</span>
            </button>
          ))}
        </div>
      </Field>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1.4fr", gap: 12 }}>
        <Field label="Amount (S$)"><input className="sgtp-input" type="number" min="0" step="0.1" placeholder="0.00" value={amount} onChange={(e) => setAmount(e.target.value)} /></Field>
        <Field label="Label (optional)"><input className="sgtp-input" style={{ fontFamily: "Inter" }} placeholder="e.g. Chinatown MRT → Sentosa" value={label} onChange={(e) => setLabel(e.target.value)} /></Field>
      </div>
      <Field label="Who's this for">
        <div style={{ display: "flex", gap: 6 }}>
          {SPLIT_TYPES.map((s) => (
            <button key={s.id} onClick={() => setSplitType(s.id)} className="sgtp-btn sgtp-mono" style={{ flex: 1, fontSize: 11, padding: "9px 4px", borderRadius: 10, cursor: "pointer", border: `1px solid ${splitType === s.id ? C.gold : C.line}`, background: splitType === s.id ? "rgba(227,177,92,0.14)" : "transparent", color: splitType === s.id ? C.gold : C.muted }}>{s.label}</button>
          ))}
        </div>
      </Field>
      <PrimaryButton onClick={add} disabled={!amount || parseFloat(amount) <= 0} icon={<Icon name="plus" size={16} />} style={{ marginBottom: 18 }}>Add expense</PrimaryButton>
      {entries.length > 0 && (
        <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
          {entries.map((e) => {
            const t = EXPENSE_TYPES.find((x) => x.id === e.type) || EXPENSE_TYPES[3];
            return (
              <div key={e.id} style={{ display: "flex", alignItems: "center", gap: 10, background: C.panel, border: `1px solid ${C.line}`, borderRadius: 10, padding: "9px 11px" }}>
                <Icon name={t.icon} size={15} color={C.muted} />
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 13, color: C.cream }}>{e.label || t.label}</div>
                  <div className="sgtp-mono" style={{ fontSize: 10.5, color: C.muted }}>{SPLIT_TYPES.find((s) => s.id === e.splitType)?.label}</div>
                </div>
                <span className="sgtp-mono" style={{ fontSize: 13, color: C.gold, flexShrink: 0 }}>{fmtMoney(e.amount)}</span>
...
- Output ONLY a raw JSON array, no markdown fences, no prose. Each item: {"day":number,"name":string,"startTime":"HH:MM","durationMin":number,"zone":"Central|Orchard|Sentosa|Mandai|Changi|Other","notes":string}. Keep notes under 12 words. Keep the array reasonably short.`;
  ...
  return (
    <div style={{ padding: "16px 16px calc(var(--sgtp-nav-h, 64px) + 36px)" }}>
...
function PlaceDetailSheet({ place, open, onClose, onSchedule, isScheduled }) {
  ...
  return (
    <div style={{ padding: "16px 16px calc(var(--sgtp-nav-h, 64px) + 36px)" }}>
      <div style={{ marginBottom: 4 }}>
        ...
        <p style={{ color: C.muted, fontSize: 13, margin: "0 0 18px" }}>Tap <Icon name="info" size={11} /> for what's actually there, + to schedule a spot onto a day.</p>
      </div>
      {ZONES.map((zone) => {
      ...
      <PlaceDetailSheet place={detailPlace} open={!!detailPlace} onClose={() => setDetailPlace(null)} onSchedule={(p) => { setDetailPlace(null); onSchedule(p); }} isScheduled={detailPlace ? scheduledNames.has(detailPlace.name) : false} />
    </div>
  );
}
...
function MapTab({ data }) {
  ...
  return (
    <div style={{ position: "absolute", top: 0, left: 0, right: 0, bottom: "var(--sgtp-nav-h, 64px)", display: "flex", flexDirection: "column", isolation: "isolate", zIndex: 0 }}>
...
// day range and the day-1 lock are enforced exactly like the manual UI, and
// every action produces a human-readable line so the user always sees what
// actually changed (or why something was skipped) before it's final.
function applyAiActions(actions, ctx) {
  ...
  for (const a of actions) {
    ...
    if (a.action === "move" || a.action === "delete") {
      ...
      if (ctx.day1Locked && day === 1) { results.push({ ok: false, label: `Can't move "${item.name}" to Day 1 — add your stay location first.` }); continue; }
      ...
      if (ctx.day1Locked && day === 1) { results.push({ ok: false, label: `Can't add "${name}" to Day 1 — add your stay location first.` }); continue; }
...
function AskAITab({ data, setData }) {
  ...
  const day1Locked = !data.homeBase?.coords;
...
TRIP JSON (day1Locked means Day 1 cannot be scheduled yet):
${JSON.stringify({ ...data, day1Locked })}
...
- Day 1 is ${day1Locked ? "LOCKED — never output day:1 in any action" : "open"}.
...
- Don't restate the changes in your visible reply — a confirmation list is shown separately, so keep your prose to context/reasoning only.`;
      ...
      let changes = [];
      if (actions.length > 0) {
        const { newItinerary, results } = applyAiActions(actions, { itinerary: data.itinerary, tripLength, day1Locked, placePool });
        changes = results;
      ...
      const msg = e.code === "no-api-key" ? "Add your Anthropic API key in Settings (key icon) first." : "Couldn't reach the planner — check connection and try again.";
...
/* =========================================================================
   HEADER + NAV
   ========================================================================= */
function Header({ tripSetup, onEdit, onSettings, onExpenses }) {
  const tripLength = tripSetup ? dayNumberFor(tripSetup.departureDate, tripSetup.arrivalDate) : 0;
  return (
...
function SingaporeTripPlanner() {
  ...
  return (
    <div ref={rootRef} className="sgtp-root" style={{ position: "relative", height: "100%", minHeight: "100vh", background: C.ink, overflow: "hidden", display: "flex", flexDirection: "column" }}>
      <GlobalStyle />
      <Header tripSetup={data.tripSetup} onEdit={() => setSetupOpen(true)} onSettings={() => setSettingsOpen(true)} onExpenses={() => setExpensesOpen(true)} />
      <div style={{ flex: 1, overflowY: activeTab === "map" || activeTab === "ai" ? "hidden" : "auto", position: "relative" }}>
        {data.tripSetup && activeTab === "itinerary" && <div className="sgtp-enter"><ItineraryTab data={data} setData={setData} recentlyAddedIds={recentlyAddedIds} removingIds={removingIds} setRemovingIds={setRemovingIds} /></div>}
        {data.tripSetup && activeTab === "places" && <div className="sgtp-enter"><PlacesTab data={data} setData={setData} onSchedule={setScheduleTarget} /></div>}
        {data.tripSetup && activeTab === "map" && <div className="sgtp-enter"><MapTab data={data} /></div>}
        {data.tripSetup && activeTab === "ai" && <div className="sgtp-enter" style={{ position: "absolute", top: 0, left: 0, right: 0, bottom: "var(--sgtp-nav-h, 64px)" }}><AskAITab data={data} setData={setData} /></div>}
      </div>
