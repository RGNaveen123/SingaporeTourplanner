...
function loadExpenses() {
  try { return JSON.parse(localStorage.getItem(EXPENSES_KEY) || "[]").map((e) => ({ ...e, type: migrateExpenseType(e.type) })); }
  catch (e) { return []; }
}
...
function ExpensesSheet({ open, onClose }) {
  ...
  useEffect(() => { if (open) { setEntries(loadExpenses()); setView("add"); } }, [open]);
  ...
  const add = () => {
    ...
    setAmount(""); setLabel(""); setScanNote(false);
  };
  ...
  const handleFileChange = async (e) => {
    ...
    try {
      ...
      if (parsed.category && EXPENSE_TYPES.some((t) => t.id === parsed.category)) setType(parsed.category);
      if (parsed.amount) setAmount(String(parsed.amount));
      if (parsed.vendor) setLabel(parsed.vendor);
      setScanNote(true);
    } catch (err) {
  ...
  return (
    <Sheet open={open} onClose={onClose} title="Expenses">
      ...
      <div style={{ display: "flex", gap: 6, marginBottom: 16 }}>
        <button onClick={() => setView("add")} className="sgtp-btn sgtp-mono" style={{ flex: 1, fontSize: 12.5, padding: "9px 4px", borderRadius: 10, cursor: "pointer", border: `1px solid ${view === "add" ? C.gold : C.line}`, background: view === "add" ? "rgba(227,177,92,0.14)" : "transparent", color: view === "add" ? C.gold : C.muted }}>+ Add expense</button>
        ...
        <>
          ...
          <Field label="Category">
            <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
              {EXPENSE_TYPES.map((t) => (
                <button key={t.id} onClick={() => setType(t.id)} className="sgtp-btn" style={{ flex: "1 1 18%", minWidth: 60, display: "flex", flexDirection: "column", alignItems: "center", gap: 4, padding: "9px 4px", borderRadius: 10, cursor: "pointer", border: `1px solid ${type === t.id ? t.color : C.line}`, background: type === t.id ? t.color + "26" : "transparent", color: type === t.id ? t.color : C.muted }}>
          ...
          <PrimaryButton onClick={add} disabled={!amount || parseFloat(amount) <= 0} icon={<Icon name="plus" size={16} />}>Add expense</PrimaryButton>
        </>
      )}
        ...
        <>
          ...
          {totals.all === 0 ? (
            ...
            <div style={{ marginBottom: 20 }}>
              <div className="sgtp-mono" style={{ fontSize: 11, color: C.muted, letterSpacing: "0.05em", textTransform: "uppercase", marginBottom: 10 }}>By category</div>
          ...
          {entries.length > 0 && (
            <>
              <div className="sgtp-mono" style={{ fontSize: 11, color: C.muted, letterSpacing: "0.05em", textTransform: "uppercase", marginBottom: 8 }}>All entries</div>
