...
const emptyExpenseForm = () => ({ type: "transport", amount: "", label: "", splitType: "personal", splitCount: 3, dateStr: new Date().toISOString().slice(0, 10) });
function ExpensesSheet({ open, onClose }) {
  ...
  const saveEntry = () => {
    ...
    if (!amt || amt <= 0) return;
    const payload = { type: form.type, amount: amt, label: form.label.trim(), splitType: form.splitType, splitCount: form.splitType === "split" ? Math.max(2, Number(form.splitCount) || 2) : null, dateStr: form.dateStr || new Date().toISOString().slice(0, 10) };
    let next;
    if (editingId) {
  ...
  const startEdit = (entry) => {
    setForm({ type: entry.type, amount: String(entry.amount), label: entry.label || "", splitType: entry.splitType, splitCount: entry.splitCount || 3, dateStr: entry.dateStr });
      ...
      {view === "add" && (
        <>
          ...
          {form.splitType === "split" && (
            <Field label="Split equally between how many people">
              <input className="sgtp-input" type="number" min="2" step="1" value={form.splitCount} onChange={(e) => setForm((f) => ({ ...f, splitCount: e.target.value }))} />
              {parseFloat(form.amount) > 0 && Number(form.splitCount) > 0 && (
                <p style={{ color: C.transit, fontSize: 12, marginTop: 6 }}>{fmtMoney(parseFloat(form.amount) / Number(form.splitCount))} per person</p>
              )}
            </Field>
          )}
            ...
            <>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                {shownEntries.map((e) => {
                  ...
                  const perPerson = e.splitType === "split" && e.splitCount ? e.amount / e.splitCount : null;
                  return (
                    <div key={e.id} style={{ display: "flex", alignItems: "center", gap: 10, background: C.panel, border: `1px solid ${C.line}`, borderRadius: 10, padding: "9px 11px" }}>
                      ...
                      <button onClick={() => startEdit(e)} className="sgtp-btn" style={{ flex: 1, minWidth: 0, textAlign: "left", background: "none", border: "none", padding: 0, cursor: "pointer" }}>
                        ...
                        <div className="sgtp-mono" style={{ fontSize: 10.5, color: C.muted }}>
                          {fmtShortDate(e.dateStr)} · {SPLIT_TYPES.find((s) => s.id === e.splitType)?.label}{perPerson ? ` · ${fmtMoney(perPerson)}/person (${e.splitCount})` : ""}
                        </div>
                      </button>
