...
function TripSetupSheet({ open, initial, homeBaseInitial, onSave, onClose, dismissable }) {
  ...
  const handleSave = () => {
    if (!valid) return;
    const coords = extractCoordsFromUrl(hbUrl);
    onSave({ tripSetup: { arrivalDate, arrivalTime, departureDate, departureTime }, homeBase: { name: hbName || "Guest house", zone: "Other", mapsUrl: hbUrl, coords } });
  };
  return (
    <Sheet open={open} onClose={onClose} title="Trip setup" dismissable={dismissable}>
      ...
      <Field label="Where you're staying (guest house / relative's place)">
        <input className="sgtp-input" style={{ fontFamily: "Inter", marginBottom: 8 }} placeholder="e.g. Aunty Su's place, Toa Payoh" value={hbName} onChange={(e) => setHbName(e.target.value)} />
        <input className="sgtp-input" style={{ fontFamily: "Inter" }} placeholder="Paste Google Maps share link (optional)" value={hbUrl} onChange={(e) => setHbUrl(e.target.value)} />
      </Field>
...
/* =========================================================================
   SCHEDULE SHEET
   ========================================================================= */
function ScheduleSheet({ open, onClose, place, tripLength, arrivalDateStr, dayTags, itinerary, onConfirm }) {
  const [day, setDay] = useState(1);
  const [startTime, setStartTime] = useState("10:00");
  const [durationMin, setDurationMin] = useState(60);
  const [notes, setNotes] = useState("");
  useEffect(() => {
    if (open && place) { setDay(1); setStartTime("10:00"); setDurationMin(place.durationMin || 60); setNotes(place.defaultNotes || place.note || ""); }
  }, [open, place]);
...
- Output ONLY a raw JSON array, no markdown fences, no prose. Each item: {"day":number,"name":string,"startTime":"HH:MM","durationMin":number,"zone":"Central|Orchard|Sentosa|Mandai|Changi|Other","notes":string}. Keep notes under 12 words. Keep the array reasonably short.`;
  ...
  return (
    <div style={{ padding: "16px 16px 100px" }}>
      <button className="sgtp-btn" onClick={handleAutoPlan} disabled={autoPlanning} style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, background: "linear-gradient(135deg, rgba(227,177,92,0.18), rgba(227,177,92,0.05))", border: `1px solid ${C.gold}`, color: C.gold, borderRadius: 14, padding: "13px 16px", fontWeight: 600, fontSize: 14, marginBottom: 18, cursor: autoPlanning ? "default" : "pointer" }}>
        {autoPlanning ? <Icon name="spinner" size={16} /> : <Icon name="sparkles" size={16} />}
        {autoPlanning ? "Planning your open days…" : `Auto-plan my open days (${openDayCount} eligible)`}
      </button>
...
function AskAITab({ data }) {
  const [messages, setMessages] = useState([{ role: "assistant", text: "Ask me anything about the plan — I can see your actual days and stops." }]);
  ...
  const send = async () => {
    ...
    try {
      const system = `You help with a real Singapore trip. Here is the current trip data as JSON — reference actual day numbers and stop names in your answers, be concise (a few sentences), and keep the budget/MRT-first, unhurried-pacing preferences in mind.\n\nTRIP JSON:\n${JSON.stringify(data)}`;
      const reply = await askClaude(text, system);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
      <div ref={scrollRef} style={{ flex: 1, overflowY: "auto", padding: "16px 16px 12px" }}>
        <h2 className="sgtp-display" style={{ fontSize: 24, color: C.cream, margin: "0 0 14px" }}>Ask AI</h2>
        {messages.map((m, i) => (
          <div key={i} className="sgtp-enter" style={{ display: "flex", justifyContent: m.role === "user" ? "flex-end" : "flex-start", marginBottom: 10 }}>
            <div style={{ maxWidth: "84%", padding: "10px 13px", borderRadius: 14, fontSize: 14, lineHeight: 1.5, background: m.role === "user" ? `linear-gradient(135deg, ${C.gold}, ${C.goldDim})` : C.panel, color: m.role === "user" ? "#12211E" : C.cream, border: m.role === "user" ? "none" : `1px solid ${C.line}`, whiteSpace: "pre-wrap" }}>{m.text}</div>
          </div>
        ))}
...
function BottomNav({ active, setActive }) {
  ...
  return (
    <div style={{ position: "absolute", left: 0, right: 0, bottom: 0, zIndex: 30, background: "rgba(10,35,32,0.92)", backdropFilter: "blur(10px)", borderTop: `1px solid ${C.line}`, padding: "8px 4px calc(env(safe-area-inset-bottom, 0px) + 8px)", display: "flex", justifyContent: "space-around" }}>
      {tabs.map((t) => {
